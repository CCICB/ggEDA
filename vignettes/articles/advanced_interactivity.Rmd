...
As an example, we'll cross-link a ggEDA visualisation and a UMAP visualisation summarising the `palmer_penguins` dataset.

### Workflow

1.  **Create your ggEDA plot**. Ensure you set `col_id` to embed a data-identifier required for cross-linking plots (in our case, we will use the unique name given to the penguins). Also set `interactive = FALSE` to force gg1 to returns a ggplot object instead of a htmlwidget (we'll add the interactivity back in after we compose all our plots). For example, lets
...
# Combine the ggEDA plot and the umap using patchwork
combined_plots <- free(gg_umap, type = "label") / free(ggEDA_penguins, side = "tb")
